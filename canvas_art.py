...
shape.set(0)
sCircle = Tkinter.Radiobutton(root, text='Circle', variable=shape, value='0')
sBox = Tkinter.Radiobutton(root, text='Box', variable=shape, value='1')
sLine = Tkinter.Radiobutton(root, text='Line', variable=shape, value='2')
sCircle.grid(row=0,column=4)
sBox.grid(row=1,column=4)
sLine.grid(row=2,column=4)
...
def drawC(x1,y1,x2,y2,f,o='#000000'):
    return canvas.create_oval(x1, y1, x2, y2, fill=f, outline=o)

def drawB(x1,y1,x2,y2,f,o='#000000'):
    return canvas.create_rectangle(x1, y1, x2, y2, fill=f, outline=o)
...
def up(event):
    ...
    new_shape = []
    if shape.get() == 0:
        new_shape = drawC(startx-r, starty-r, startx+r, starty+r, tk_color_string)
    elif shape.get() == 1:
        new_shape = drawB(startx+w,starty+l,startx-w,starty-l, tk_color_string)
    elif shape.get() == 2:
        new_shape = canvas.create_line(startx,starty,event.x,event.y)
